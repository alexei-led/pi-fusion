...
export function renderPartialPanelReport(
  ...
  const sections: ReportSection[] = [
    ...
    ...(merger
      ? [
          {
            title: 'Coverage Map' as const,
            content:
              'Partial coverage only; surviving facet outputs are listed below.',
          },
          {
            title: 'Combined Answer' as const,
            content: candidateText || 'No usable panel output.',
          },
          {
            title: 'Gaps' as const,
          ...
          {
            title: 'Conflicts At Seams' as const,
            content: 'Not synthesized because the composer quorum was not met.',
          },
        ]
      : [
          {
            title: 'Consensus' as const,
            content: 'Not synthesized because the panel quorum was not met.',
          },
          {
            title: 'Disagreements' as const,
            content: 'Not synthesized because the judge did not run.',
          },
          {
            title: 'Unique Insights' as const,
            content: candidateText || 'No usable panel output.',
          },
          {
            title: 'Blind Spots' as const,
            content:
              'Unavailable perspectives and absent cross-panel synthesis can hide important issues.',
          },
        ]),
    {
      title: 'Recommendation',
      content:
        'Use the surviving panel output as incomplete evidence, not a final fusion recommendation.',
    },
    {
      title: 'Risks',
    ...
    },
    {
      title: 'Next Step',
      content:
        'Inspect the unavailable perspectives; after this terminal run, manually start a new /fusion run if full coverage is needed.',
    },
    { title: 'Run Metadata', content: formatRunMetadata(input.run) },
  ];
  const runDetails = formatRunDetails({
