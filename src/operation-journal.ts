...
export class FusionOperationJournal {
  ...
  private path(operationId: string, suffix: string): string {
    return join(
      this.directory,
      `${requestDigest(operationId).slice(7)}.${suffix}`,
    );
  }
  ...
  claim(
    ...
    try {
      ...
      if (
        !saved ||
        saved.digest !== digest ||
        saved.requestDigest !== callerDigest
      ) {
        throw new Error(
          'Fusion operationId was already claimed with a different request digest.',
          { cause: error },
        );
      }
  ...
  pendingPreflights(): FusionPreflight[] {
    ...
    for (const file of readdirSync(this.directory).filter((name) =>
      ...
      if (
        ...
        this.path(raw.operationId, 'intent') !== join(this.directory, file)
      )
        throw new Error('Corrupt Fusion preflight intent.');
      const saved = this.readIntent(raw.operationId);
      if (saved?.recovery && !this.cancelled(raw.operationId))
        pending.push(saved.recovery);
    }
    return pending;
  }
  ...
  beginDispatch(operationId: string, digest?: string): boolean {
    ...
    if (intent && digest !== undefined && intent.digest !== digest)
      throw new Error(
        'Fusion dispatch digest does not match its durable intent.',
      );
    ...
    if (
      admission.digest !== undefined &&
      digest !== undefined &&
      admission.digest !== digest
    )
      throw new Error('Fusion dispatch digest does not match its admission.');
  ...
  cancel(
    ...
    this.ensureDirectory();
    try {
      writeFileSync(this.path(operationId, 'cancel'), 'cancelled', {
        flag: 'wx',
        mode: 0o600,
        flush: true,
      });
    ...
    const intent = this.readIntent(operationId);
    const legacyOrUnknownLaunch =
      intent?.version !== 2 && (intent !== undefined || mayHaveDispatched);
    this.admit({
      version: 1,
      operationId,
  ...
  releaseBeforeLaunch(operationId: string, runId?: string): void {
    ...
    if (runId !== undefined && this.preflight(operationId)?.runId !== runId)
      return;
    try {
      unlinkSync(this.path(operationId, 'intent'));
    } catch (error: unknown) {
  ...
  cancelled(operationId: string): boolean {
    try {
      readFileSync(this.path(operationId, 'cancel'));
      return true;
    } catch (error: unknown) {
  ...
  lookup(operationId: string): FusionOperationEvidence {
    ...
    if (this.cancelled(operationId))
      return {
        ...
        replaySafe: false,
        cancellationRequested: true,
        neverStarted: admission?.state === 'never-started',
      };
    if (admission?.state === 'never-started')
      throw new Error(
        'Fusion cancellation admission has no durable cancellation fence.',
      );
    return {
      ...identity,
      state: intent || admission ? 'pending' : 'absent',
      replaySafe: !intent && !admission,
    };
  }
  ...
  private admit(value: Admission): Admission {
    try {
      ...
      if (!saved)
        throw new Error('Fusion admission disappeared during arbitration.', {
          cause: error,
        });
      return saved;
    }
  ...
  private publish(operationId: string, suffix: string, value: unknown): void {
    ...
    try {
      ...
      try {
        ...
        /* An unpublished temporary file cannot authorize dispatch. */
      }
    }
  }
  ...
  private readIntent(operationId: string): OperationIntent | undefined {
    ...
    if (
      ...
      !saved.digest ||
      (saved.version !== undefined && saved.version !== 2) ||
      (saved.requestDigest !== undefined &&
        typeof saved.requestDigest !== 'string')
    )
      throw new Error('Corrupt Fusion operation intent.');
    if (
      saved.recovery !== undefined &&
      (!isPreflight(saved.recovery) ||
        saved.recovery.args.operationId !== operationId ||
        saved.recovery.args.requestDigest !== saved.digest)
    )
      throw new Error('Corrupt Fusion preflight request.');
    return {
      operationId,
      digest: saved.digest,
      ...(saved.version === 2 ? { version: 2 } : {}),
      ...(typeof saved.requestDigest === 'string'
        ? { requestDigest: saved.requestDigest }
        : {}),
      ...(saved.recovery !== undefined ? { recovery: saved.recovery } : {}),
    };
  }
  ...
  private readAdmission(operationId: string): Admission | undefined {
    ...
    if (
      ...
      saved.version !== 1 ||
      saved.operationId !== operationId ||
      (saved.state !== 'dispatching' && saved.state !== 'never-started') ||
      (saved.digest !== undefined && typeof saved.digest !== 'string') ||
      (saved.requestDigest !== undefined &&
        typeof saved.requestDigest !== 'string')
    )
      throw new Error('Corrupt Fusion operation admission.');
    return {
      version: 1,
      operationId,
      state: saved.state,
      ...(typeof saved.digest === 'string' ? { digest: saved.digest } : {}),
      ...(typeof saved.requestDigest === 'string'
        ? { requestDigest: saved.requestDigest }
        : {}),
    };
  ...
  private readJson(operationId: string, suffix: string): unknown {
    try {
      return JSON.parse(
        readFileSync(this.path(operationId, suffix), 'utf8'),
      ) as unknown;
    } catch (error: unknown) {
...
function isPreflight(value: unknown): value is FusionPreflight {
  if (
    ...
    !isRecord(value.args) ||
    value.argsDigest !== requestDigest(value.args)
  )
    return false;
  ...
  return (
    ...
    (args.outputContract === undefined ||
      args.outputContract === 'plan-review-v1') &&
    args.panel === undefined &&
    args.timeoutOverrides === undefined
  );
}
