...
export function isRetiredKernelProof(
  value: unknown,
  expectedNative?: NativeOperationIdentity,
): boolean {
  if (
    !isRecord(value) ||
    !isRecord(value.nativeOperation) ||
    !nonBlank(value.nativeOperation.operationId) ||
    !nonBlank(value.nativeOperation.digest)
  )
    return false;
  if (
    expectedNative &&
    (value.nativeOperation.operationId !== expectedNative.operationId ||
      value.nativeOperation.digest !== expectedNative.digest)
  )
    return false;
  ...
  if (
    !identity ||
    !sameBinding(identity, binding) ||
    !isRecord(proof) ||
    proof.kind !== 'darwin-coalition-retired' ||
    !sameBinding(proof, binding) ||
    !isoDate(proof.observedAt)
  )
    return false;
...
function sameBinding(value: unknown, expected: KernelBinding): boolean {
  ...
  return (
    ...
    binding.bootId === expected.bootId
  );
}
...
function kernelIdentity(value: unknown): KernelIdentity | undefined {
  ...
  if (
    ...
    !isRecord(value) ||
    value.version !== 1 ||
    value.backend !== 'darwin-resource-coalition-v1' ||
    !positiveDecimal(value.coalitionId) ||
    !isRecord(value.leader) ||
    !positiveInteger(value.leader.pid) ||
    !positiveDecimal(value.leader.uniqueId) ||
    typeof value.leader.pidVersion !== 'number' ||
    !Number.isInteger(value.leader.pidVersion) ||
    value.leader.pidVersion < 0 ||
    value.leader.pidVersion > 0xffff_ffff
  )
    return undefined;
  return {
    ...binding,
    version: 1,
    backend: 'darwin-resource-coalition-v1',
    coalitionId: value.coalitionId,
    leader: {
      pid: value.leader.pid,
      uniqueId: value.leader.uniqueId,
      pidVersion: value.leader.pidVersion,
    },
  };
}
...
function sameIdentity(left: KernelIdentity, right: KernelIdentity): boolean {
  return (
    ...
    left.leader.pid === right.leader.pid &&
    left.leader.uniqueId === right.leader.uniqueId &&
    left.leader.pidVersion === right.leader.pidVersion
  );
}
