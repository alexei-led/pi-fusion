...
import {
  FusionRunStore,
  type FusionRunStorePersistence,
...
const capabilities = {
  ...
  processTreeOwnership: {
    ...
    requestMode: 'kernel',
    routes: ['single-async', 'parallel-data'],
  },
};
...
class NativeRuntime implements FusionRpcClientLike {
  ...
  async spawn(params: object): Promise<unknown> {
    ...
    this.runId =
      this.route === 'parallel-data' ? 'native-panel' : 'native-judge';
    if (this.loseReply) throw new Error('spawn reply lost');
    return {
      ...this.identity,
      runId: this.runId,
      effectiveExecutionLifetime: lifetime,
      effectiveExecutionOwnership: { mode: 'kernel' },
      executionRoute: this.route,
    };
  }
  ...
  payload(): unknown {
    return {
      ...
      results: [{ agent: 'panelist', output: 'Answer', success: true }],
    };
  }
}
...
function proof(rpc: NativeRuntime) {
  ...
  const binding = {
    operationId: `kernel:${String(rpc.identity.operationId)}`,
    requestDigest: `kernel:${String(rpc.identity.digest)}`,
  ...
  const identity = {
    ...
    ...binding,
    coalitionId: rpc.route === 'parallel-data' ? '123' : '124',
    leader: {
      pid: 1234,
      uniqueId: rpc.route === 'parallel-data' ? '123456' : '123457',
      pidVersion: 0,
    },
  };
  return {
    ...
    state: 'observed',
    runId: rpc.runId,
    runnerProcessInstanceId: 'native-instance',
    observedAt: 1,
...
function admissionProcess(
  ...
  const child = spawn(
    ...
    [
      ...
      'jiti/register',
      fileURLToPath(
        new URL('../support/shared-admission-worker.ts', import.meta.url),
      ),
      cwd,
      operationId,
      mode,
  ...
  const ready = new Promise<void>((resolve, reject) => {
    ...
    child.once('exit', (code) => {
      if (code !== 0 && code !== 78)
        reject(new Error(`Admission worker exited ${code}: ${stderr}`));
    });
  });
...
function recoveryProcess(t: TestContext, cwd: string, mode: string) {
  const child = spawn(
    ...
    [
      ...
      'jiti/register',
      fileURLToPath(
        new URL('../support/stale-recovery-worker.ts', import.meta.url),
      ),
      cwd,
      mode,
    ],
  ...
  const exited = new Promise<void>((resolve, reject) => {
    ...
    child.once('exit', (code) =>
      code === 0
        ? resolve()
        : reject(new Error(`Recovery worker exited ${code}: ${stderr}`)),
    );
  });
...
for (const boundary of ['lookup', 'spawn', 'lookup-refreshed']) {
  test(`two processes preserve judge admission and cancellation after delayed panel ${boundary}`, {
    timeout: 20_000,
  }, async (t) => {
    ...
    assert.equal(
      (
        await initial.request('start', {
          operationId: 'shared-recovery',
          prompt: 'Review',
          digest: 'caller',
    ...
    const panelLookup = {
      ...((await native.lookup()) as Record<string, unknown>),
      statusPayload: {
        runId: 'native-panel',
        state: 'completed',
        processTerminalProof: native.proof,
        results: [
          { agent: 'panelist', output: 'One', success: true },
          { agent: 'panelist', output: 'Two', success: true },
        ],
      },
    };
    await writeFile(
      join(cwd, 'native-fixture.json'),
      JSON.stringify({ capabilities, panelLookup }),
    );
    const delayed = recoveryProcess(t, cwd, boundary);
    await delayed.waiting;
    await recoveryProcess(t, cwd, 'advance').exited;
    const admitted = new FusionRunStore({
      directory: join(cwd, 'runs'),
    }).getActiveRun();
    ...
    const current = new FusionRunStore({
      directory: join(cwd, 'runs'),
    }).getActiveRun();
    ...
    const events: unknown[] = (
      await readFile(join(cwd, 'native-events.jsonl'), 'utf8')
    )
      .trim()
      .split('\n')
    ...
    assert.equal(
      events.filter(
        (event) => isRecord(event) && event.method === 'judge-spawn',
      ).length,
      1,
    );
    assert.equal(
      events.some((event) => isRecord(event) && event.method === 'stop'),
      false,
    );
    for (const event of events)
      if (isRecord(event) && event.method === 'cancel') {
        assert.ok(isRecord(event.params));
        assert.equal(event.params.operationId, `${run.id}:judge`);
      }
    const rpc = new NativeRuntime();
    rpc.identity = {
      operationId: admitted?.spawnIntent?.requestId,
      digest: admitted?.spawnIntent?.requestDigest,
    };
...
for (const mode of ['rpc', 'direct'] as const) {
  test(`independent ${mode} processes arbitrate one active run and keep the loser replayable`, {
    timeout: 20_000,
  }, async (t) => {
    const cwd = await mkdtemp(join(tmpdir(), 'fusion-shared-admission-'));
    onTestFinished(() => rm(cwd, { recursive: true, force: true }));
    const workers = ['first', 'second'].map((id) =>
      admissionProcess(t, cwd, id, mode),
    );
    ...
    const starts: unknown[] = await Promise.all(
      ['first', 'second'].map(
        async (id) =>
          JSON.parse(
            await readFile(join(cwd, `${id}.result.json`), 'utf8'),
          ) as unknown,
      ),
    );
    assert.equal(
      starts.filter(
        (reply) =>
          isRecord(reply) &&
          (reply.success === true || reply.status === 'started'),
      ).length,
      1,
    );
    const launches = (await readFile(join(cwd, 'launches.jsonl'), 'utf8'))
      .trim()
      .split('\n');
    assert.equal(launches.length, 1);
    const params: unknown = JSON.parse(required(launches[0]));
    ...
    assert.equal(
      new FusionRunStore({ directory: join(cwd, 'runs') }).getRunById(winner.id)
        ?.phase,
      'cancelled',
    );
    ...
    if (mode === 'rpc')
      ...
      assert.equal(
        (
          await restored.orchestrator.startRun(
            { prompt: 'Concurrent', executionLifetime: lifetime },
            new FakePi().createContext(cwd),
          )
        ).status,
        'started',
      );
  ...
  test(`${mode} admission survives process death before the ordinary run snapshot`, {
    timeout: 20_000,
  }, async (t) => {
    ...
    rpc.lookup = async () =>
      rpc.identity
        ? lookup()
        : { state: 'absent', operationId: run.spawnIntent?.requestId };
...
test('unbounded launch persists identity and survives old elapsed deadlines', async (_t) => {
  ...
  const started = await orchestrator.startRun(
    { prompt: 'Review', executionLifetime: lifetime },
  ...
  assert.deepEqual(
    rpc.spawns[0] &&
      (rpc.spawns[0] as { executionLifetime: unknown }).executionLifetime,
    lifetime,
  );
  assert.equal('timeoutMs' in required(rpc.spawns[0]), false);
...
for (const fault of ['after-initial-write', 'before-intent-write'] as const) {
  for (const outcome of ['launch', 'cancel', 'continue'] as const) {
    test(`direct public tool survives ${fault} and recovers by ${outcome}`, async (_t) => {
      ...
      const boot = async () => {
        ...
        vi.spyOn(pi.events, 'emit').mockImplementation(
          (event: string, payload: unknown) => {
              ...
              case 'lookup':
                ...
                reply = rpc.identity
                  ...
                  : Promise.resolve({
                      ...
                      state: outcome !== 'cancel' ? 'absent' : 'pending',
                    });
                break;
              case 'cancel':
                assert.ok(isRecord(params));
            ...
            void reply.then((data) =>
              emit(subagentsRpcReplyChannel(requestId), {
                version: 1,
                requestId,
                method,
                success: true,
        ...
        onTestFinished(() => pi.emitLifecycle('session_shutdown', {}, ctx));
      ...
      let writes = 0;
      const mockedWrite = vi
        .spyOn(DurableRunSnapshotStore.prototype, 'write')
        .mockImplementation(function (
          ...
          if (this.directory === directory) {
            writes += 1;
            if (writes === 2 && fault === 'before-intent-write')
              throw new Error('interrupted before intent update');
          }
          ...
          if (
            ...
            writes === 1 &&
            fault === 'after-initial-write'
          )
            throw new Error('interrupted after initial publication');
        });
      ...
      assert.equal(
        run.spawnIntent.requestDigest,
        requestDigest(run.spawnIntent.params),
      );
      assert.equal(rpc.spawns.length, 0);
      if (outcome !== 'continue')
        await first.pi.emitLifecycle('session_shutdown', {}, first.ctx);
      const restarted = outcome === 'continue' ? first : await boot();
      if (outcome === 'continue')
        await restarted.pi.runCommand('fusion', 'status', restarted.ctx);
      if (outcome === 'cancel')
        await restarted.pi.runCommand('fusion', 'stop', restarted.ctx);
      ...
      if (outcome !== 'cancel') {
        ...
        assert.deepEqual(rpc.identity, {
          operationId: run.spawnIntent.requestId,
          digest: run.spawnIntent.requestDigest,
        });
        await restarted.pi.runCommand('fusion', 'status', restarted.ctx);
        assert.equal(rpc.spawns.length, 1);
        rpc.statusValue = 'completed';
        ...
        await restarted.pi.runCommand('fusion', 'status', restarted.ctx);
        assert.equal(
          new FusionRunStore({ directory }).getLastRunSummary()?.phase,
          'done',
        );
      }
    });
...
test('lost native spawn reply is adopted across restart without duplicate spawn', async (_t) => {
  ...
  const first = new FusionOrchestrator({
    rpc,
    runStore: firstStore,
    loadConfig: async () => config,
  });
  ...
  const started = await first.startRun(
    { prompt: 'Review', operationId: 'operation', executionLifetime: lifetime },
    ctx,
  );
  assert.equal(started.status, 'started');
  ...
  const restored = new FusionOrchestrator({
    rpc,
    runStore: store,
    loadConfig: async () => config,
  });
...
for (const lateCallback of [false, true]) {
  test(`shared-store refresh uses the new run profile${lateCallback ? ' after late old-run callbacks' : ''}`, async (t) => {
    ...
    const profileA: FusionConfig = {
      defaultProfile: 'quality',
      profiles: {
        quality: {
          panel,
          judge: { agent: 'judge-a', model: 'provider/model-a' },
        },
      },
    };
    const profileB: FusionConfig = {
      defaultProfile: 'quality',
      profiles: {
        quality: {
          panel,
          judge: { agent: 'judge-b', model: 'provider/model-b' },
        },
      },
    };
    class SharedRuntime extends NativeRuntime {
      ...
      override payload(): unknown {
        return {
          ...
          ...(this.proof ? { processTerminalProof: this.proof } : {}),
          results:
            this.route === 'parallel-data'
              ? [
                  { agent: 'panelist', output: 'First', success: true },
                  { agent: 'panelist', output: 'Second', success: true },
                ]
              : [
                  {
                    agent: 'judge-a',
    ...
    if (lateCallback) {
      ...
      oldCompletion = observer.orchestrator.handleSubagentComplete({
        runId: oldPanelId,
      });
      await waiting;
    ...
    assert.equal(
      (
        await first.request('start', {
          operationId: 'profile-second',
          prompt: 'Second',
          executionLifetime: lifetime,
          digest: 'caller-second',
        })
      ).success,
      true,
    );
    ...
    if (lateCallback) {
      ...
      assert.deepEqual(
        new FusionRunStore({ directory: join(cwd, 'runs') }).getRunById(
          secondRun.id,
        ),
        secondRun,
      );
      assert.equal(
        (
          await observer.orchestrator.handleSubagentComplete({
            runId: oldPanelId,
          })
        ).status,
        'ignored',
    ...
    assert.equal(
      observer.store.getActiveRun()?.spawnIntent?.requestDigest,
      digest,
    );
    assert.deepEqual(
      observer.store.getActiveRun()?.profileSnapshot,
      secondRun.profileSnapshot,
    );
    assert.equal(rpc.spawns.length, 4);
  });
}

test('wrapper terminal and cancel receipt are not native tree exit', async (_t) => {
  const rpc = new NativeRuntime();
  const store = new FusionRunStore();
  const orchestrator = new FusionOrchestrator({
    rpc,
  ...
  const terminalProof = summary?.processTerminalProof as
    | { runId: string }
    | undefined;
  assert.equal(terminalProof?.runId, summary?.id);
});
...
test('cancellation during deferred completion status wins the terminal transition', async (t) => {
  ...
  const completionWaiting = new Promise<void>((resolve) => {
    completionEntered = resolve;
  });
  const cancellationWaiting = new Promise<void>((resolve) => {
  ...
  const cancelling = fixture.orchestrator.cancelActiveRun(
    new FakePi().createContext(cwd),
  );
  ...
  assert.equal(
    new FusionRunStore({ directory: join(cwd, 'runs') }).getLastRunSummary()
      ?.phase,
    'cancelled',
  );
});
...
for (const cancellation of ['terminal', 'durable-marker'] as const) {
  test(`${cancellation} cancellation is honored after completion status resumes`, async (t) => {
    ...
    if (cancellation === 'terminal') {
      const result = await fixture.orchestrator.cancelActiveRun(
        new FakePi().createContext(cwd),
      );
      assert.equal(result.status, 'cancelled');
    } else {
      new FusionOperationJournal(
        join(cwd, '.pi', 'fusion', 'operations'),
      ).cancel(operationId, true);
      assert.equal(
        fixture.store.getActiveRun()?.cancellationRequested,
        undefined,
      );
    }
...
test('owned root requires kernel retirement even when a workflow claims closed dispatch', async (_t) => {
  const rpc = new NativeRuntime();
  const store = new FusionRunStore();
  const orchestrator = new FusionOrchestrator({
    rpc,
  ...
  rpc.proof = {
    ...workflow,
    dispatchClosed: true,
    children: [{ ...proof(rpc), state: 'pending' }],
  };
  await orchestrator.getStatusReport();
  assert.ok(store.getActiveRun());
...
test('public RPC preserves durable identity, rejects changed replay, and fences late starts', async (_t) => {
  ...
  onTestFinished(() => orchestrator.dispose());
  const { registerFusionRpc, FUSION_RPC_REQUEST_EVENT, fusionRpcReplyEvent } =
    await import('../../src/fusion-rpc.js');
  onTestFinished(
    registerFusionRpc({
      events: pi.events,
      orchestrator,
  ...
  assert.equal(rpc.spawns.length, 1);
  assert.equal(
    (await request('start', { ...input, prompt: 'Different' })).success,
    false,
  );
  const fence = await request('cancel', { operationId: 'late-operation' });
  assert.equal(fence.success, true);
  assert.equal(
    (await request('start', { ...input, operationId: 'late-operation' }))
      .success,
    false,
  );
  assert.equal(rpc.spawns.length, 1);
...
test('closed panel proof cannot acknowledge cancellation of an unresolved judge launch', async (_t) => {
  const rpc = new NativeRuntime();
  ...
  onTestFinished(() => orchestrator.dispose());
  const result = await orchestrator.cancelActiveRun(
    new FakePi().createContext(),
  );
...
test('empty process group cannot prove escaped descendants exited', async (_t) => {
  const rpc = new NativeRuntime();
  const store = new FusionRunStore();
  const orchestrator = new FusionOrchestrator({
    rpc,
...
test('explicit dispatch refuses the current group-only ownership capability', async (_t) => {
  const rpc = new NativeRuntime();
  rpc.ping = async () => ({
    capabilities: {
      ...capabilities,
  ...
  assert.deepEqual(advertised.processTreeOwnership, {
    version: 1,
    scope: 'posix-process-group',
    escapedDescendants: 'unverified',
  });
...
test('unchanged unbounded activity polls do not grow durable session history', async (_t) => {
  const rpc = new NativeRuntime();
  const pi = new FakePi();
...
test('native workflow stage does not accept a wrapper process proof', async (_t) => {
  const rpc = new NativeRuntime();
  const store = new FusionRunStore();
  const orchestrator = new FusionOrchestrator({
    rpc,
...
test('cancellation arriving during native lookup prevents an absent-intent replay', async (_t) => {
  const rpc = new NativeRuntime();
  rpc.loseReply = true;
  const store = new FusionRunStore();
  const orchestrator = new FusionOrchestrator({
...
function rpcHarness(
  ...
  loadConfig = async () => configuration,
...
for (const boundary of ['ping', 'config'] as const) {
  test(`RPC preflight interrupted at ${boundary} resumes on restore and late original adopts one run`, async (t) => {
    ...
    const input = {
      operationId: `recover-${boundary}`,
      prompt: 'Frozen review',
      profile: 'quality',
    ...
    assert.equal(
      responseData(
        await first.request('status', { operationId: input.operationId }),
      ).state,
      'pending',
    );
    ...
    assert.equal(rpc.spawns.length, 1);
    assert.equal(
      new FusionRunStore({ directory: join(cwd, 'runs') }).getRunByOperationId(
        input.operationId,
      )?.id,
      runId,
    );
...
test('simultaneous RPC preflight replays freeze one profile and one native identity', async (t) => {
  ...
  rpc.ping = () =>
    new Promise((resolve) => {
      gates.push(() => resolve({ capabilities }));
    });
  const first = rpcHarness(t, candidate.sessionCwd, rpc);
  const changedConfig: FusionConfig = {
    defaultProfile: 'quality',
    profiles: {
      quality: {
        panel: [{ id: 'changed', agent: 'changed-panelist' }],
        judge: { agent: 'changed-judge' },
      },
    },
  };
  ...
  const input = {
    operationId: 'concurrent-preflight',
    prompt: 'Frozen prompt',
    profile: 'quality',
    digest: 'frozen-caller',
    executionLifetime: lifetime,
    cwd: candidate.candidateCwd,
    reviewedCommit: candidate.reviewedCommit,
  };
  const starts = [
    first.request('start', input),
    second.request('start', input),
    third.request('start', input),
  ];
  assert.equal(gates.length, 3);
  for (const release of gates) release();
  for (const reply of await Promise.all(starts))
    assert.equal(reply.success, true);
  ...
  assert.deepEqual(run.reviewContext, {
    cwd: input.cwd,
    reviewedCommit: input.reviewedCommit,
  });
  assert.equal(
    responseData(
      await second.request('status', { operationId: input.operationId }),
    ).requestDigest,
    input.digest,
  );
  assert.equal(
    (await second.request('start', { ...input, prompt: 'Changed' })).success,
    false,
  );
  first.dispose();
  third.dispose();
...
test('cancel during restarted preflight fences simultaneous replay and late original', async (t) => {
  ...
  const restarted = rpcHarness(t, cwd, rpc);
  const restoring = restarted.orchestrator.restore(
    new FakePi().createContext(cwd),
  );
  const replay = restarted.request('start', input);
  assert.equal(gates.length, 3);
  const cancellation = responseData(
    await restarted.request('cancel', { operationId: input.operationId }),
  );
  ...
  assert.equal(rpc.spawns.length, 0);
  assert.equal(
    new FusionRunStore({ directory: join(cwd, 'runs') }).getActiveRun(),
    undefined,
  );
  ...
  assert.equal(
    responseData(
      await final.request('status', { operationId: input.operationId }),
    ).neverStarted,
    true,
  );
...
test('restart after initial snapshot publication replays its frozen panel intent', async (t) => {
  ...
  onTestFinished(() => rm(cwd, { recursive: true, force: true }));
  const rpc = new NativeRuntime();
  const first = rpcHarness(t, cwd, rpc, config, undefined, {
    appendEntry() {
      throw new Error('interrupted after durable snapshot publication');
    },
  });
  const input = {
    operationId: 'initial-snapshot',
    prompt: 'Frozen',
    profile: 'quality',
    digest: 'caller',
    executionLifetime: lifetime,
  ...
  rpc.lookup = async () =>
    rpc.identity
      ? originalLookup()
      : { state: 'absent', operationId: run.spawnIntent?.requestId };
  const restarted = rpcHarness(t, cwd, rpc, config, async () => {
    throw new Error('configuration must remain frozen');
  });
  await restarted.orchestrator.restore(new FakePi().createContext(cwd));
  assert.equal(restarted.store.getActiveRun()?.id, run.id);
  assert.equal(restarted.store.getActiveRun()?.panelRunId, 'native-panel');
  assert.deepEqual(
    restarted.store.getActiveRun()?.profileSnapshot,
    run.profileSnapshot,
  );
  assert.equal(rpc.spawns.length, 1);
});
...
test('recoverable intent with dispatch admission and missing run never infers an absent child', async (t) => {
  ...
  rpc.ping = () =>
    new Promise((resolve) => {
      release = () => resolve({ capabilities });
    });
  const first = rpcHarness(t, cwd, rpc);
  const input = {
    operationId: 'missing-construction',
    prompt: 'Review',
    digest: 'caller',
    executionLifetime: lifetime,
  };
  const starting = first.request('start', input);
  const journal = new FusionOperationJournal(
    join(cwd, '.pi', 'fusion', 'operations'),
  );
  assert.equal(
    journal.beginDispatch(input.operationId, requestDigest(input)),
    true,
  );
  rpc.ping = async () => ({ capabilities });
  const restarted = rpcHarness(t, cwd, rpc);
...
test('restored preflights retry unavailable native service and drain after prior completion', async (t) => {
  ...
  originalRpc.ping = () =>
    new Promise((resolve) => {
      releases.push(() => resolve({ capabilities }));
    });
  const original = rpcHarness(t, cwd, originalRpc);
  const starts = ['queued-one', 'queued-two'].map((operationId) =>
    original.request('start', {
      operationId,
      prompt: 'Frozen',
      digest: `caller-${operationId}`,
      executionLifetime: lifetime,
    }),
  );
  ...
  rpc.spawn = async (params) => {
    const reply = await spawn(params);
    launched();
    return reply;
  };
  ...
  const nextLaunch = new Promise<void>((resolve) => {
    launched = resolve;
  });
  vi.advanceTimersByTime(2_000);
  await nextLaunch;
  ...
  assert.equal(
    new FusionRunStore({ directory: join(cwd, 'runs') }).getActiveRun()?.id,
    restarted.store.getActiveRun()?.id,
  );
  assert.equal(rpc.spawns.length, 2);
...
test('RPC cancellation before claim proves no launch across restart and delayed start', async (t) => {
  ...
  assert.deepEqual(responseData(await first.request('status', selector)), {
    ...selector,
    state: 'absent',
    replaySafe: true,
  });
...
test('RPC cancellation after claim beats delayed native admission and retains caller digest', async (t) => {
  ...
  const input = {
    ...
    prompt: 'Delayed',
    executionLifetime: lifetime,
    digest: 'caller-frozen-digest',
  };
  const starting = first.request('start', input);
  assert.ok(resolvePing);
  const beforeCancel = responseData(
    await first.request('status', { operationId: input.operationId }),
  );
  assert.equal(beforeCancel.state, 'pending');
  assert.equal(beforeCancel.replaySafe, false);
  const cancellation = responseData(
    await first.request('cancel', { operationId: input.operationId }),
  );
  assert.equal(cancellation.neverStarted, true);
  ...
  const status = responseData(
    await restored.request('status', { operationId: input.operationId }),
  );
  assert.equal(status.neverStarted, true);
...
test('RPC restart with admitted identity but no bound run stays unresolved', async (t) => {
  ...
  const evidence = responseData(
    await restarted.request('cancel', { operationId: 'admitted' }),
  );
  ...
  const status = responseData(
    await restarted.request('status', { operationId: 'admitted' }),
  );
...
test('strict Fusion refuses ownership without both native data routes', async (_t) => {
  const rpc = new NativeRuntime();
  rpc.ping = async () => ({
    capabilities: {
      ...capabilities,
...
test('strict agreement policy refusal happens before admission while service lookup remains available', async (_t) => {
  const rpc = new NativeRuntime();
  const store = new FusionRunStore();
  const agreement: FusionConfig = {
    defaultProfile: 'quality',
    profiles: {
      quality: {
        ...required(config.profiles.quality),
  ...
  assert.equal(result.status, 'failed');
  if (result.status === 'failed')
    assert.match(result.error, /stopWhenPanelAgrees/);
  assert.equal(rpc.spawns.length, 0);
  assert.equal(store.getActiveRun(), undefined);
  assert.ok((await orchestrator.executionCapabilities()).executionLifetime);
});

test('native ownership route downgrade remains an unresolved launch', async (_t) => {
  const rpc = new NativeRuntime();
...
test('owned panel and direct judge close under separate native identities bound to one caller', async (t) => {
  ...
  rpc.proof = {
    ...retired,
    kernelProof: { ...retired.kernelProof, status: 'active' },
  };
  await fixture.orchestrator.getStatusReport();
  assert.equal(fixture.store.getActiveRun()?.phase, 'judge');
  rpc.proof = retired;
  await fixture.orchestrator.getStatusReport();
  const completed = responseData(
    await fixture.request('result', { operationId: input.operationId }),
  );
  assert.deepEqual(completed.callerOutput, {
  ...
  assert.ok(
    typeof closure === 'object' &&
      closure !== null &&
      'callerBinding' in closure &&
      'children' in closure,
  );
  assert.deepEqual(closure.callerBinding, {
  ...
  for (const child of children) {
    assert.ok(
      typeof child === 'object' &&
        child !== null &&
        'nativeOperation' in child &&
        'kernelBinding' in child,
    );
...
async function candidateRepositories(t: TestContext) {
  ...
  return {
    sessionCwd,
    candidateCwd,
    sessionCommit: head(sessionCwd),
    reviewedCommit: head(candidateCwd),
  };
}
...
test('candidate context uses repository B for panel and restored judge while Pi stays in A', async (t) => {
  ...
  const input = {
    operationId: 'candidate-review',
    digest: 'caller-candidate-digest',
    prompt: 'Review',
    executionLifetime: lifetime,
    ...context,
  };
  ...
  assert.ok(
    panelSpawn &&
      'cwd' in panelSpawn &&
      'operationId' in panelSpawn &&
      'digest' in panelSpawn &&
      'worktree' in panelSpawn,
  );
  ...
  const admitted: unknown = JSON.parse(
    await readFile(
      join(candidate.sessionCwd, 'runs', '.admissions', 'root.json'),
      'utf8',
    ),
  );
  assert.ok(
    isRecord(admitted) &&
      isRecord(admitted.data) &&
      isRecord(admitted.data.spawnIntent) &&
      isRecord(admitted.data.spawnIntent.params),
  );
  ...
  assert.equal(
    (
      await first.request('start', {
        ...
        reviewedCommit: candidate.sessionCommit,
      })
    ).success,
    false,
  );
  ...
  rpc.statusValue = 'completed';
  rpc.proof = proof(rpc);
  await restored.orchestrator.restore(
    new FakePi().createContext(candidate.sessionCwd),
  );
  ...
  assert.ok(
    judgeSpawn &&
      'cwd' in judgeSpawn &&
      'agent' in judgeSpawn &&
      'worktree' in judgeSpawn &&
      'operationId' in judgeSpawn &&
      'digest' in judgeSpawn,
  );
  ...
  const {
    operationId: _judgeId,
    digest: judgeDigest,
    ...judgeParams
  } = judgeSpawn;
  assert.equal(
    judgeDigest,
    requestDigest({ params: judgeParams, reviewContext: context }),
  );
  rpc.statusValue = 'completed';
  rpc.proof = proof(rpc);
  ...
  assert.deepEqual(
    new FusionRunStore({
      directory: join(candidate.sessionCwd, 'runs'),
    }).getRunByOperationId(input.operationId)?.reviewContext,
    context,
  );
});
...
test('candidate HEAD mismatch rejects before native admission and leaves cancellation available', async (t) => {
  ...
  const response = await fixture.request('start', {
    operationId,
    digest: 'caller-digest',
    prompt: 'Review',
    executionLifetime: lifetime,
  ...
  assert.deepEqual(
    responseData(await fixture.request('status', { operationId })),
    { operationId, state: 'absent', replaySafe: true },
  );
  assert.equal(
    responseData(await fixture.request('cancel', { operationId })).neverStarted,
    true,
  );
});
...
test('candidate HEAD drift cannot produce an accepted report and does not prevent cancellation', async (t) => {
  ...
  const fixture = rpcHarness(t, candidate.sessionCwd);
  await fixture.request('start', {
    operationId: 'drifting-candidate',
    digest: 'caller-digest',
    prompt: 'Review',
    executionLifetime: lifetime,
    cwd: candidate.candidateCwd,
  ...
  await assert.rejects(
    fixture.orchestrator.getStatusReport(),
    /does not match reviewedCommit/,
  );
  assert.equal(fixture.store.getActiveRun()?.phase, 'panel');
  assert.equal(fixture.store.getLastRunSummary(), undefined);
  assert.equal(fixture.rpc.spawns.length, 1);
  const cancelled = responseData(
    await fixture.request('cancel', { operationId: 'drifting-candidate' }),
  );
  assert.equal(cancelled.cancelled, true);
  assert.equal(fixture.store.getLastRunSummary()?.phase, 'cancelled');
});
...
test('review context requires a complete absolute directory and immutable commit pair', async (t) => {
  ...
  for (const [index, context] of [
    ...
    const response = await fixture.request('start', {
      operationId: `invalid-context-${index}`,
      prompt: 'Review',
      executionLifetime: lifetime,
      digest: 'caller-digest',
      ...context,
...
test('lost-launch replay retains candidate context and native digest after restart', async (t) => {
  ...
  rpc.loseReply = false;
  rpc.lookup = async () => ({
    operationId: rpc.identity?.operationId,
    state: 'absent',
  });
  ...
  assert.deepEqual(restored.store.getActiveRun()?.reviewContext, {
    cwd: candidate.candidateCwd,
    reviewedCommit: candidate.reviewedCommit,
  });
...
test('candidate verification cannot be redirected by inherited Git repository variables', async (t) => {
  ...
  try {
    const response = await fixture.request('start', {
      operationId: 'git-environment-candidate',
      digest: 'caller-digest',
      prompt: 'Review',
      executionLifetime: lifetime,
      cwd: candidate.candidateCwd,
...
test('fixture Git commands isolate inherited hook selectors from a disposable sentinel repository', async (t) => {
  ...
  const beforeHead = fixtureGit([
    '-C',
    sentinel.sessionCwd,
    'rev-parse',
    'HEAD',
  ]);
  const beforeRefs = fixtureGit([
    '-C',
    sentinel.sessionCwd,
    'show-ref',
    '--heads',
  ...
  const originalGitEnvironment = Object.entries(process.env).filter(([name]) =>
  ...
  try {
    ...
    assert.equal(
      fixtureGit(['-C', sentinel.sessionCwd, 'rev-parse', 'HEAD']),
      beforeHead,
    );
    assert.equal(
      fixtureGit(['-C', sentinel.sessionCwd, 'show-ref', '--heads']),
      beforeRefs,
    );
    assert.deepEqual(
      await readFile(join(sentinelGitDir, 'config')),
      beforeConfig,
    );
    assert.deepEqual(
      await readFile(join(sentinelGitDir, 'index')),
      beforeIndex,
    );
    assert.equal(
      fixtureGit(['-C', sentinel.sessionCwd, 'status', '--short']),
      '',
    );
  } finally {
    for (const name of Object.keys(process.env))
      if (name.startsWith('GIT_')) delete process.env[name];
    for (const [name, value] of originalGitEnvironment)
