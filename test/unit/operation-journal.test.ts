...
test('operation journal fences same identity across restart and rejects parameter drift', async (_t) => {
  ...
  assert.throws(
    () => restarted.claim('operation', 'sha256:second'),
...
test('prelaunch cancellation is durable and prevents a late original launch', async (_t) => {
  const dir = await mkdtemp(join(tmpdir(), 'fusion-journal-'));
  onTestFinished(() => rm(dir, { recursive: true, force: true }));
  new FusionOperationJournal(dir).cancel('late');
  assert.equal(
    new FusionOperationJournal(dir).claim('late', 'sha256:late'),
    'cancelled',
  );
});

test('confirmed prelaunch refusal releases its claim without removing a cancellation fence', async (_t) => {
  const dir = await mkdtemp(join(tmpdir(), 'fusion-journal-'));
  onTestFinished(() => rm(dir, { recursive: true, force: true }));
...
test('dispatch admission cannot be changed into never-started evidence', async (_t) => {
  const dir = await mkdtemp(join(tmpdir(), 'fusion-journal-'));
  onTestFinished(() => rm(dir, { recursive: true, force: true }));
  ...
  assert.throws(
    () => launcher.beginDispatch('admitted', 'different'),
    /digest/,
  );
  ...
  assert.equal(
    new FusionOperationJournal(dir).lookup('admitted').neverStarted,
    false,
  );
  assert.equal(launcher.beginDispatch('admitted', 'internal'), false);
});

test('cancel marker crash can finish cancellation but cannot authorize replay', async (_t) => {
  const dir = await mkdtemp(join(tmpdir(), 'fusion-journal-'));
  onTestFinished(() => rm(dir, { recursive: true, force: true }));
  const journal = new FusionOperationJournal(dir);
  journal.claim('interrupted-cancel', 'internal', 'caller');
  await writeFile(
    join(dir, `${requestDigest('interrupted-cancel').slice(7)}.cancel`),
    'cancelled',
  );
  const restarted = new FusionOperationJournal(dir);
  ...
  assert.equal(
    restarted.beginDispatch('interrupted-cancel', 'internal'),
    false,
  );
  assert.equal(restarted.cancel('interrupted-cancel').neverStarted, true);
});

test('legacy intent and corrupt admission remain fail closed', async (_t) => {
  const dir = await mkdtemp(join(tmpdir(), 'fusion-journal-'));
  onTestFinished(() => rm(dir, { recursive: true, force: true }));
  await writeFile(
    join(dir, `${requestDigest('legacy').slice(7)}.intent`),
    JSON.stringify({ operationId: 'legacy', digest: 'old-digest' }),
  );
  const journal = new FusionOperationJournal(dir);
  assert.equal(journal.cancel('legacy').neverStarted, false);
  assert.equal(journal.lookup('legacy').replaySafe, false);
  await writeFile(
    join(dir, `${requestDigest('corrupt').slice(7)}.admission`),
    '{',
  );
