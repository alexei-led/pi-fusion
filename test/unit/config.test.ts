...
test('loadFusionConfig prefers trusted project config over global config', async (_t) => {
...
test('loadFusionConfig ignores project config when the project is untrusted', async (_t) => {
  const root = await makeTempDir();
  const cwd = join(root, 'project');
  const agentDir = join(root, 'agent');
  await writeJson(
...
test('loadFusionConfig resolves Claude alias shorthand in panel and judge models', async (_t) => {
  const root = await makeTempDir();
  const cwd = join(root, 'project');
  const agentDir = join(root, 'agent');
...
test('loadFusionConfig rejects duplicate Claude handles across global and project aliases', async (_t) => {
  const root = await makeTempDir();
  const cwd = join(root, 'project');
  const agentDir = join(root, 'agent');
...
test('loadFusionConfig fails on malformed JSON', async (_t) => {
  const root = await makeTempDir();
  const agentDir = join(root, 'agent');
...
test('loadFusionConfig fails on invalid config shape', async (_t) => {
  const root = await makeTempDir();
  const agentDir = join(root, 'agent');
...
test('runFusionInit writes a trusted project template', async (_t) => {
  const root = await makeTempDir();
...
test('runFusionInit skips untrusted projects and protects existing config without confirmation', async (_t) => {
  const root = await makeTempDir();
...
test('Claude alias rewriting preserves every new profile and member field', async (_t) => {
  const root = await makeTempDir();
  const cwd = join(root, 'project');
  const agentDir = join(root, 'agent');
