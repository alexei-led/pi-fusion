...
import type { ExtractPanelResultsSuccess } from '../../src/result-extract.js';
    ...
    const result = reconcilePanelResults(
      {
        ok: true,
        outputs: [
          { index: 1, agent: 'panel-agent', output: `${field} event output` },
        ],
        failures: [
          { index: 0, agent: 'panel-agent', summary: 'stale event failure' },
        ],
...
test('uses complete status steps to restore failures omitted by compact events', () => {
  ...
  if (result.ok) {
    assert.deepEqual(
      result.outputs.map(({ index }) => index),
      [2, 3],
    );
    assert.deepEqual(
      result.failures.map(({ index }) => index),
      [0, 1],
    );
  }
});
...
test('rejects explicit empty terminal status arrays rather than treating them as absent', () => {
  for (const statusPayload of [
    ...
    if (!result.ok)
      assert.match(result.error.message, /status described 0 of 4/);
  }
});
