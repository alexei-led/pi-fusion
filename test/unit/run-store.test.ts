...
import {
  ...
  durableSnapshotFileName,
} from '../../src/durable-run-store.js';
import {
...
test('terminal admission survives snapshot failure and fences stale writers and sessions', (_t) => {
  ...
  const fault = vi
    ...
    .mockImplementation(
      (key: string, data: unknown, exclusive?: boolean, expected?: unknown) => {
        if (key === run.id) throw new Error('crash after terminal admission');
        write(key, data, exclusive, expected);
      },
    );
  assert.throws(
    () => first.cancelRun(run.id, { report: 'cancelled' }),
  ...
  restarted.restoreFromEntries([
    {
      type: 'custom',
      customType: FUSION_RUN_ENTRY_TYPE,
      data: { ...run, updatedAt: 1000 },
    },
  ]);
  assert.equal(restarted.getActiveRun(), undefined);
  assert.equal(restarted.getRunById(run.id)?.phase, 'cancelled');
  const next = restarted.startRun({
    id: 'next',
    prompt: 'Next',
    profileName: 'quality',
  });
...
test('revision compaction keeps one tip snapshot per run', (_t) => {
  ...
  for (let index = 0; index < 25; index += 1)
    store.updateRun(run.id, {
      panelRunId: `panel-${index}`,
      updatedAt: index + 2,
    });
  const revisions = readdirSync(
    join(directory, '.revisions', durableSnapshotFileName(run.id)),
  );
...
for (const successor of ['chained', 'unrelated'] as const) {
  test(`recreating a compacted revision is ${successor === 'chained' ? 'accepted' : 'rejected'} from its successor`, (_t) => {
    ...
    internal.readRevision = (key: string) => {
      ...
      if (++calls === 1) {
        ...
        writeFileSync(
          join(revisionDirectory, '3.json'),
          `${JSON.stringify({
            version: 1,
            key,
            sequence: 3,
            previous:
              successor === 'chained' ? requestDigest(stale) : 'another-write',
    ...
    if (successor === 'chained') store.write('race', stale, false, first);
      ...
      assert.throws(
        () => store.write('race', stale, false, first),
        DurableRunConflictError,
      );
  });
}

test('an older unfinished legacy snapshot is never hidden by a newer terminal run', (_t) => {
  ...
  assert.throws(
    () => store.startRun({ prompt: 'Another', profileName: 'quality' }),
    /already active/,
  );
  store.cancelRun('older', { report: 'Cancelled old run' });
  assert.equal(
    new FusionRunStore({ directory }).getRunById('older')?.phase,
    'cancelled',
  );
});

test('an aliased active snapshot cannot override an immutable terminal identity', (_t) => {
  ...
  writeFileSync(
    join(directory, 'zzz.json'),
    JSON.stringify({ ...run, updatedAt: run.updatedAt + 1_000 }),
  );
  const restored = new FusionRunStore({ directory });
...
test('unknown unfinished runs and malformed admission chains fail closed', (_t) => {
  ...
  assert.throws(
    () => conflicted.startRun({ prompt: 'Another', profileName: 'quality' }),
    /restore is blocked/,
  );
  ...
  writeFileSync(
    join(
      directory,
      '.admissions',
      `after-${durableSnapshotFileName('unknown')}`,
    ),
    JSON.stringify({
      version: 1,
      key: 'orphan',
  ...
  assert.match(
    new FusionRunStore({ directory }).getRestoreError() ?? '',
    /Unreachable Fusion admission/,
  );
});
...
test('FusionRunStore persists deferred failed-slot recovery metadata', () => {
  ...
  assert.deepEqual(
    restored.getLastRunSummary()?.recovery?.failedPanelIndices,
    [1, 3],
  );
});
...
test('FusionRunStore keeps project snapshots across sessions and restart', () => {
  ...
  try {
    ...
    assert.deepEqual(
      JSON.parse(
        readFileSync(join(directory, required(snapshotFiles[0])), 'utf8'),
      ),
      first.getActiveRun(),
    );
