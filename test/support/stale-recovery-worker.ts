...
const fixture: unknown = JSON.parse(
  readFileSync(join(cwd, 'native-fixture.json'), 'utf8'),
);
...
const barrier = async () => {
  ...
  while (!existsSync(join(cwd, 'release-stale'))) {
    if (Date.now() > deadline)
      throw new Error('Stale reply barrier timed out.');
    await delay(5);
  }
...
const rpc: FusionRpcClientLike = {
  async ping() {
    return { capabilities: fixture.capabilities };
  },
  async lookup(params) {
    if (params.operationId === panelLookup.operationId) {
      if (!waited && mode.startsWith('lookup')) await barrier();
      if (!waited && mode === 'spawn')
        return { state: 'absent', operationId: params.operationId };
      return panelLookup;
    }
    const saved: unknown =
      judge ?? JSON.parse(readFileSync(join(cwd, 'judge-launch.json'), 'utf8'));
    ...
    return {
      ...
      statusPayload: { runId: 'native-judge', state: 'running' },
    };
  },
  async spawn(params) {
    ...
    judge = params;
    appendFileSync(
      join(cwd, 'native-events.jsonl'),
      `${JSON.stringify({ method: 'judge-spawn', params })}\n`,
    );
  ...
  async cancel(params) {
    appendFileSync(
      join(cwd, 'native-events.jsonl'),
      `${JSON.stringify({ method: 'cancel', params })}\n`,
    );
  ...
  async stop(params) {
    appendFileSync(
      join(cwd, 'native-events.jsonl'),
      `${JSON.stringify({ method: 'stop', params })}\n`,
    );
    return {};
  },
  async interrupt() {
