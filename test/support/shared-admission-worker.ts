...
DurableRunSnapshotStore.prototype.admit = (key, data, predecessor) => {
...
const rpc: FusionRpcClientLike = {
  async ping() {
    return { capabilities: JSON.parse(capabilities) as unknown };
  },
  async spawn(params) {
    ...
    return {
      ...
      executionRoute: 'parallel-data',
    };
  },
  async lookup(params) {
    return { state: 'absent', ...params };
  },
  async cancel(params) {
    return { state: 'cancelled', neverStarted: true, ...params };
  },
  async status() {
...
if (mode === 'direct')
  response = await orchestrator.startRun(
    { prompt: 'Concurrent', executionLifetime: { mode: 'unbounded' } },
    ctx,
  );
else {
...
writeFileSync(
  join(cwd, `${operationId}.result.json`),
